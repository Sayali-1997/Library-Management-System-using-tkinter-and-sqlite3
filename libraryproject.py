...
homepage.title("SM Library")
back=Canvas(homepage)
back.pack(expand=True, fill=BOTH)
image=PhotoImage(file="book1.png")
back.img=image
...
def adminrequest():
      ...
      def submit():
            login=e1.get()
            password=e2.get()
            if(login=="shrutika@sm.com" and password=="shruti123"):
                  adminrequest.destroy()
                  adminpage()
            else:
...
def userpage():
      ...
      Radiobutton(user, text="View all the books", font="Calibri 15", fg="Dark green", bg="Light pink", command=assign1, value=1).grid(padx=350, pady=30)
...
def user():
      homepage.withdraw()
      userpage()
home=Label(back, text="SM Library", font="Isocteur 30 bold underline", fg="Orange", bg="#222").grid(padx=400, pady=80)
adminbutton=Button(back, command=admin, text="Admin", font="Calibri 15 bold", fg="Dark green", padx=10, pady=10, bg="Light grey", border=4).grid(padx=350, pady=80)
userbutton=Button(back, command=user, text="User", font="Calibri 15 bold", fg="Dark green", padx=17, pady=10, bg="Light grey", border=4).grid(padx=40)
homepage.mainloop()
